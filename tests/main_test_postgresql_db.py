...
from sqlalchemy import create_engine, select, String,insert, update, delete, func
...
def add_todo_tasks():
    with Session(engine) as conn:
        # max_id = conn.execute(func.max(ToDo.id)).scalar()
...
def add_shoping_tasks():
    with engine.connect() as conn:
        # max_id = conn.execute(func.max(Shopping.id)).scalar()
        # if max_id == None:
        #     max_id = 0
